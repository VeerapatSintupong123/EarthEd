...
export default function PreTestPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (course && courseStatus) {
      ...
      if (!isNaN(chapterUser) && !isNaN(chapterNow)) {
        if (chapterNow !== chapterUser || !currentUser.match("pre")) {
          Swal.fire({
  ...
  const updateUser = async () => {
    const updatedStatus = { ...courseStatus, current: "learn" };
  ...
  const handleSubmit = () => {
    ...
    exam.forEach((question, index) => {
      const answerData = `${date},${subject},Pre-Test,${question.question},${question.answer},${email},${answers[index]}`;
      data += `\n${answerData}`;
  ...
  return (
    <main className="flex flex-col items-center min-h-[100vh] p-3 overflow-hidden bg-slate-200">
      <h1 className="text-3xl font-semibold mb-6 mt-5">Pre-Test</h1>
